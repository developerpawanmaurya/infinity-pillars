...
import PortfolioTiltGridSection from '@/components/portfolio-variants/PortfolioTiltGridSection.jsx';
...
// Comparison build — Tilt Grid variant. Throwaway route, see
// PortfolioVariantSwitcher for context. Delete once a variant ships.
const PortfolioPreviewGridPage = () => {
  useEffect(() => {
  ...
  return (
    <>
      <Helmet>
        <title>Portfolio Preview — Tilt Grid</title>
        ...
        <section className="pt-40 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="max-w-4xl">
            ...
            <p className="text-lg text-muted-foreground max-w-xl">
              Tilt Grid — an asymmetric bento grid with parallax + magnetic tilt on hover.
            </p>
          </motion.div>
        </section>

        <PortfolioTiltGridSection projects={portfolioProjects} />

        <Footer />
      </div>

      <PortfolioVariantSwitcher active="grid" />
    </>
  );
};

export default PortfolioPreviewGridPage;
