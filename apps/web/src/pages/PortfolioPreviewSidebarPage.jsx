...
import PortfolioSidebarSelectorSection from '@/components/portfolio-variants/PortfolioSidebarSelectorSection.jsx';
...
// Comparison build — Sidebar Selector variant. Throwaway route, see
// PortfolioVariantSwitcher for context. Delete once a variant ships.
const PortfolioPreviewSidebarPage = () => {
  useEffect(() => {
  ...
  return (
    <>
      <Helmet>
        <title>Portfolio Preview — Sidebar Selector</title>
        ...
        <section className="pt-40 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="max-w-4xl">
            ...
            <p className="text-lg text-muted-foreground max-w-xl">
              Sidebar Selector — click a project name, the detail pane morphs.
            </p>
          </motion.div>
        </section>

        <PortfolioSidebarSelectorSection projects={portfolioProjects} />

        <Footer />
      </div>

      <PortfolioVariantSwitcher active="sidebar" />
    </>
  );
};

export default PortfolioPreviewSidebarPage;
