...
// Comparison build — Variant C (Editorial Index + Cursor Preview). Throwaway
// route, see PortfolioVariantSwitcher for context. Delete once a variant ships.
const PortfolioPreviewIndexPage = () => {
  useEffect(() => {
  ...
  return (
    <>
      <Helmet>
        <title>Portfolio Preview — Index</title>
        ...
        <section className="pt-40 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="max-w-4xl">
            ...
            <p className="text-lg text-muted-foreground max-w-xl">
              Variant C — an editorial index. Hover a title for a live preview.
            </p>
          </motion.div>
        </section>

        <PortfolioIndexSection projects={portfolioProjects} />

        <Footer />
      </div>

      <PortfolioVariantSwitcher active="index" />
    </>
  );
};

export default PortfolioPreviewIndexPage;
