...
import PortfolioScrubSlideshowSection from '@/components/portfolio-variants/PortfolioScrubSlideshowSection.jsx';
...
// Comparison build — Scrub Slideshow variant. Throwaway route, see
// PortfolioVariantSwitcher for context. Delete once a variant ships.
const PortfolioPreviewSlideshowPage = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Portfolio Preview — Scrub Slideshow</title>
        ...
        <section className="pt-40 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} className="max-w-4xl">
            ...
            <p className="text-lg text-muted-foreground max-w-xl">
              Scrub Slideshow — scroll crossfades between projects, or jump via the rail.
            </p>
          </motion.div>
        </section>

        <PortfolioScrubSlideshowSection projects={portfolioProjects} lenisRef={lenisRef} />

        <Footer />
      </div>

      <PortfolioVariantSwitcher active="slideshow" />
    </>
  );
};

export default PortfolioPreviewSlideshowPage;
